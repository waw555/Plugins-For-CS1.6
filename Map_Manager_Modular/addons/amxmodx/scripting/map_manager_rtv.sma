...
public plugin_init()
{
    ...
    g_hForwards[VOTE_RTV] = CreateMultiForward("mapm_vote_rtv", ET_IGNORE, FP_CELL);
...
public clcmd_rtv(id)
{
    ...
    if(need_votes <= 0) {
        //if(g_bRTVSound){
		//    g_bRTVSound=false;
		//    ExecuteForward(g_hForwards[VOTE_RTV],ret, id);
		//}
        ExecuteForward(g_hForwards[VOTE_RTV],ret, id);
        set_task(1.0, "mapm_start_vote_rtv",0,_,_,"a", 1);
