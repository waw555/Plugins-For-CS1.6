// 0.2.4.1 - Добавлена возможность сыграть еще 1 раунд после голосования и dhud сообщение о последнем раунде
#include <amxmodx>
#include <amxmisc>
#include <map_manager>
...
#define VERSION "0.2.4.1 - 01.09.2025"
...
public plugin_init()
{
    ...
    g_pCvars[CHANGE_TYPE] = register_cvar("mapm_change_type", "1"); // 0 - after end vote, 1 -  change in one round, 2 - in round end, 3 - after end map
    ...
    g_hForwards[VOTE_REPEAT] = CreateMultiForward("mapm_vote_repeat", ET_IGNORE, FP_CELL);
    g_hForwards[VOTE_LAST_ROUND] = CreateMultiForward("mapm_vote_last_round", ET_IGNORE, FP_CELL);
...
// plugin = номер плагина, params = id игрока
...
public event_newround()
{
    ...
    if(is_vote_finished() && g_eLastRoundState == LRS_Not){
        g_eLastRoundState = LRS_Last;
        ExecuteForward(g_hForwards[VOTE_LAST_ROUND],ret, 0);
        set_task(2.0, "event_last_round",1596,_,_,"a",2);
        /*new players[32], pnum; get_players(players, pnum, "ch");
        set_dhudmessage(255, 0, 0, -1.0, 0.01, 0, 0.0, 1800.0, 0.0, 0.0);
        show_dhudmessage(0, "%L", LANG_PLAYER, "MAPM_LASTROUND");
        new nextmap[MAPNAME_LENGTH]; get_string(NEXTMAP, nextmap, charsmax(nextmap));
        set_dhudmessage(0, 255, 0, -1.0, 0.05, 0, 0.0, 1800.0, 0.0, 0.0);
        show_dhudmessage(0, "%L %s", LANG_PLAYER, "MAPM_NEXTMAP", nextmap);*/
        /*for(new i, id; i < pnum; i++) {
            id = players[i];
            show_dhudmessage(id, "%L", id, "MAPM_LASTROUND");
        }*/
    }
...
public event_last_round()
{
	set_dhudmessage(255, 0, 0, -1.0, 0.01, 0, 0.0, 1800.0, 0.0, 0.0);
	show_dhudmessage(0, "%L", LANG_PLAYER, "MAPM_LASTROUND");
	new nextmap[MAPNAME_LENGTH]; get_string(NEXTMAP, nextmap, charsmax(nextmap));
	strtoupper(nextmap);
	set_dhudmessage(0, 255, 0, -1.0, 0.04, 0, 0.0, 1800.0, 0.0, 0.0);
	show_dhudmessage(0, "%L %s", LANG_PLAYER, "MAPM_NEXTMAP_DHUD", nextmap);
}
...
public mapm_vote_finished(const map[], type, total_votes)
{
    ...
    if(can_be_extend) {
        ...
        if(!total_votes && extend_map_no_votes) {
            set_task(4.0, "mapm_no_body_vote",0,_,_,"a", 1);
        }
        else {
            set_task(4.0, "mapm_map_extend",0,_,_,"a", 1);
        }
    ...
    if(get_num(LAST_ROUND)) {
    ...
    } else if(get_num(CHANGE_TYPE) == CHANGE_IN_ONE_ROUND) {	
        g_fOldTimeLimit = get_float(TIMELIMIT);
        set_float(TIMELIMIT, 0.0);
        if(get_players_num() == 0)
		{
            log_amx("[vote_finished]: no players, no last round");
            g_eLastRoundState = LRS_Last;
		}else{
            log_amx("[vote_finished]: have players, have last round");
            g_eLastRoundState = LRS_Not;
		}
        log_amx("[vote_finished]: next round is last - saved timelimit is %f", g_fOldTimeLimit);
        ...
        log_amx("[vote_finished]: map change after vote");
        intermission();
        ...
        log_amx("[vote_finished]: map change next round - saved timelimit is %f", g_fOldTimeLimit);
    }

    return 0;
}

public mapm_map_extend(){
    client_print_color(0, print_team_default, "%s^1 %L %L.", g_sPrefix, LANG_PLAYER, "MAPM_MAP_EXTEND", num, LANG_PLAYER, lang);
    ExecuteForward(g_hForwards[VOTE_REPEAT],ret, 0);
}

public mapm_no_body_vote(){
    num = get_num(EXTENDED_TIME);
